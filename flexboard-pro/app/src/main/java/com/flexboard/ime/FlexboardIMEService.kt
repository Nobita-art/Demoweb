 ...
 *   • Change onCreateInputView() to skip AccessGate.cachedState() check.
 *   • Make allowsKeyboard always true regardless of AccessState.
 *   • Remove the approvalRevokedReceiver or prevent it from swapping the view.
 *   • Skip the soft-refresh in onStartInputView that detects revocation.
 *   • Return any keyboard View (non-lock) when the user is Blocked/PlanExpired.
 *
...
class FlexboardIMEService : InputMethodService() {
    ...
    private fun makeImeLockView(): View {
        val tv = android.widget.TextView(this).apply {
            text = "🔒  FlexBoard Pro is locked\n\n" +
                    "Tap here to open FlexBoard and send your\n" +
        ...
        tv.setOnClickListener {
            try {
                startActivity(
                    Intent(this, com.flexboard.ui.MainActivity::class.java)
    ...
    private val prefsListener = SharedPreferences.OnSharedPreferenceChangeListener { _, key ->
        ...
        when (key) {
            SettingsStore.KEY_FONT_PATH, SettingsStore.KEY_FONT_BOLD, SettingsStore.KEY_FONT_ITALIC -> {
                ...
                com.flexboard.utils.FontManager.invalidateCache()
    ...
    private val approvalRevokedReceiver = object : android.content.BroadcastReceiver() {
        override fun onReceive(context: android.content.Context?, intent: android.content.Intent?) {
            if (intent?.action != com.flexboard.security.ApprovalRefreshWorker.ACTION_APPROVAL_REVOKED) return
            try {
                // v1.11 — invalidate the cache so any view that's already
                // about to consult AccessGate during this same frame falls
                // through to the lock branch. Without this the prefs may
                // still hold "Approved" from before the worker overwrote
                // them on a slow device.
                com.flexboard.security.ApprovalGate.invalidateCache(applicationContext)
                ...
                // the lock view because AccessGate now returns the locked
                // state from the freshly-updated SharedPrefs.
    ...
    override fun onCreate() {
        ...
        // Clipboard watcher is started in FlexboardApp so it stays alive even
        ...
        // Strict injector — auto-type ONLY succeeds when the FlexBoard IME is
    ...
    override fun onCreateInputView(): View {
        // v1.11 — hard gate via AccessGate (combines security check +
        // GitHub approval/plan + local trial). Returns the lock view
        // unless allowsKeyboard is true. Android won't let us return
        // null here (the OS requires every IME to provide a view), so
        // we hand back a disabled placeholder. The user can still
        // long-press space to switch to a different keyboard.
        val state = try { com.flexboard.security.AccessGate.cachedState(this) }
            catch (_: Throwable) { null }
        if (state == null || !state.allowsKeyboard) {
        ...
        val view = KeyboardView(
            ...
            // Tools-grid tiles route their tap to a specific section so
            // "Themes" opens Theme & Appearance directly (was opening the
            // Home page — issue #1 in user's review).
            onOpenSection = { section -> openSettingsTo(section) },
            readBeforeCursor = {
                // Return RAW (normalized) text — callers want to do plain ASCII
                // matching against macros / suggestions. The field may contain
            ...
            onPickFile = {
                // Signal AutoTypeScreen to auto-launch the file picker on open,
                // then navigate to the Auto-Type tab in the main app.
                com.flexboard.utils.SettingsStore.prefs(this)
                    .edit().putBoolean(com.flexboard.utils.SettingsStore.KEY_AUTO_PICK_FILE, true).apply()
                openSettingsTo("autotype")
            }
        )
        keyboardView = view
        return view
    ...
    override fun onStartInputView(info: EditorInfo?, restarting: Boolean) {
        ...
        // v1.11 — Soft approval refresh. The cached state is the source of
        // truth for the view we just rendered, but if it's older than the
        // ApprovalGate TTL (5 min) we kick off an async re-fetch right
        // here. If GitHub now says Blocked / NotApproved / PlanExpired the
        // background coroutine fires the revoked broadcast which our
        // [approvalRevokedReceiver] picks up and swaps to the lock view
        // mid-session. Combined with the worker, this means revocation
        // propagates within seconds the moment the user opens any field
        // — no more "keep typing until I switch keyboards".
        try {
            if (com.flexboard.security.ApprovalGate.isCacheStale(this)) {
                scope.launch(Dispatchers.IO) {
                    val newState = try {
                        com.flexboard.security.ApprovalGate.evaluate(this@FlexboardIMEService, force = true)
                    } catch (_: Throwable) { null }
                    val locked = newState is com.flexboard.security.ApprovalGate.State.Blocked ||
                                 newState is com.flexboard.security.ApprovalGate.State.NotApproved ||
                                 newState is com.flexboard.security.ApprovalGate.State.PlanExpired
                    if (locked) {
                        com.flexboard.security.ApprovalGate.invalidateCache(this@FlexboardIMEService)
                        try {
                            sendBroadcast(
        ...
        } catch (_: Throwable) { /* never crash field bind */ }
    }
    ...
    private fun handleKey(key: KeyDef) {
        ...
        when (key.type) {
            ...
            KeyType.SPACE -> {
                // Macro detection works on RAW (ASCII) text, since macros are
                // stored in plain text. We normalize stylized glyphs back to
                // their ASCII equivalents before scanning.
                val rawBefore = UnicodeFonts.normalize(
                    ic.getTextBeforeCursor(64, 0)?.toString() ?: ""
                )
                val expansion = MacroEngine.checkExpansion(rawBefore)
                if (expansion != null) {
                    val (token, replacement) = expansion
                    // Compute the actual UTF-16 length of the token AS IT
                    // EXISTS IN THE FIELD (it might be stylized, taking
                    // 2 char units per ASCII char). Walk back through the
                    // field text matching one glyph at a time.
                    val fieldLen = matchedFieldLengthForRaw(ic, token.length)
                    if (fieldLen > 0) ic.deleteSurroundingText(fieldLen, 0)
                    ic.commitText(stylize(replacement) + stylize(" "), 1)
                    sentenceBuf.append(replacement).append(' ')
                } else {
                    ic.commitText(stylize(" "), 1)
                    sentenceBuf.append(' ')
                }
                val lastWord = rawBefore.takeLastWhile { it.isLetterOrDigit() }
            ...
            KeyType.CUT   -> { ic.performContextMenuAction(android.R.id.cut);   sentenceBuf.clear() }
    ...
    private fun commitSuggestion(word: String) {
        ...
        val partial = rawBefore.takeLastWhile { it.isLetterOrDigit() }
        if (partial.isNotEmpty()) {
            val n = matchedFieldLengthForRaw(ic, partial.length)
            if (n > 0) ic.deleteSurroundingText(n, 0)
        }
        ic.commitText(stylize("$word "), 1)
    ...
    private fun updateSuggestions(prefix: String) {
        if (!SettingsStore.prefs(this).getBoolean(SettingsStore.KEY_SUGGESTIONS, true)) {
            ...
            currentEmojiSuggestions = emptyList()
        ...
        scope.launch {
            ...
            // Emoji suggestions: when the user is mid-word, match against the
            // partial prefix (so "love" lights up ❤️ 💕 the moment it's typed).
            // When the prefix is blank (just hit space), fall back to the
            // previous word so emojis still surface on the next keystroke.
            val emojis = when {
                prefix.isNotBlank() -> EmojiSuggestions.forKeyword(prefix, 2)
                prevWord != null -> EmojiSuggestions.forKeyword(prevWord, 2)
                else -> emptyList()
            }
            currentSuggestions = list
            currentEmojiSuggestions = emojis
            keyboardView?.refreshSuggestions()
        }
    }

    /**
     * Insert an emoji at the cursor. Unlike [commitSuggestion] this does NOT
     * delete the partial word the user is typing — they explicitly chose the
     * emoji as a *companion* to that word ("love" + ❤️). Emojis bypass the
