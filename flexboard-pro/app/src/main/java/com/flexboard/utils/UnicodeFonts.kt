...
object UnicodeFonts {
    ...
    val STYLES: List<Style> by lazy {
        listOf(
            ...
            // ===== Math / Leet styles =====
            Style("leet_basic",   "Leet 1337 Basic",  "H3ll0 W0rld")    { toLeetBasic(it) },
            Style("leet_pro",     "H4ck3r Pr0",       "5y5T3m H4cK")    { toLeetPro(it) },
            Style("leet_cyber",   "Cy83r @tt@ck",     "Cy83r @tt@ck")   { toLeetCyber(it) },
            Style("leet_shadow",  "5h4d0w M0d3",      "D4rk 5h4d0w")    { toLeetShadow(it) },
            Style("leet_matrix",  "M47r1x C0d3",      "M47r1x W0rld")   { toLeetMatrix(it) },
            Style("triple_echo",  "TTTriiippp EEEccc","TTTriiippp")      { toTripleEcho(it) },
     ...
     * suggestions / autocorrect / macro detection still work after the user
    ...
    // ===== MATH / LEET TRANSFORMS =====

    fun toLeetBasic(s: String): String {
        val sb = StringBuilder(s.length)
        for (c in s) {
            sb.append(when (c.lowercaseChar()) {
                'a' -> "4"; 'e' -> "3"; 'i' -> "1"
                'o' -> "0"; 's' -> "5"; 't' -> "7"
    ...
    fun toLeetCyber(s: String): String {
        val sb = StringBuilder(s.length)
        for (c in s) {
            sb.append(when (c.lowercaseChar()) {
                'a' -> "@"; 'b' -> "8"; 'e' -> "3"
    ...
    fun toLeetShadow(s: String): String {
        val sb = StringBuilder(s.length)
        for (c in s) {
            sb.append(when (c.lowercaseChar()) {
                'a' -> "4"; 'b' -> "8"; 'e' -> "€"
    ...
    fun toLeetMatrix(s: String): String {
        val sb = StringBuilder(s.length)
        for (c in s) {
            sb.append(when (c.lowercaseChar()) {
                'a' -> "4"; 'b' -> "8"; 'c' -> "("
    ...
    fun toTripleEcho(s: String): String {
        val sb = StringBuilder(s.length * 3)
        for (c in s) {
            if (c == ' ') { sb.append("   ") } else {
                val u = c.uppercaseChar()
                repeat(3) { sb.append(u) }
            }
        }
        return sb.toString()
    }
