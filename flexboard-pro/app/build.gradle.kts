...
//      KEYSTORE_BASE64 into  app/keystore/flexboard-release.jks  and
...
//          storeFile=keystore/flexboard-release.jks
//          storePassword=...
//          keyAlias=...
//          keyPassword=...
...
fun computeKeystoreSha(jks: File, password: String, alias: String): String {
    return try {
        ...
        println("[FlexBoard obf] Failed to read keystore: ${e.message}")
        ""
    }
}
...
val generateObfConstants = tasks.register("generateObfConstants") {
    ...
    doLast {
        val pkgName = "com.flexboard"
        val plaintexts = linkedMapOf(
            "APPROVAL_URL"     to "https://keyboard.kraza.qzz.io/approvals.json",
            "WHATSAPP_NUMBER"  to "923001677853",
            "OWNER_NAME"       to "Kashif Raza",
            "OWNER_TEAM"       to "ATF Team",
            "INSTAGRAM_URL"    to "https://www.instagram.com/iamkashif_raza",
            "FACEBOOK_URL"     to "https://www.facebook.com/oyeimraza",
            "WA_CHANNEL_URL"   to "https://whatsapp.com/channel/0029Vb7qlHtChq6DFRZQGe0V",
            "WA_COMMUNITY_URL" to "https://chat.whatsapp.com/Ft0cI6Kc02b91julyAkNKO?s=cl&p=a&mlu=4",
            "LICENSE_LINE"     to "Kashif Raza \u00B7 ATF Team. All rights reserved.",
        ...
        if (canSignRelease) {
            ...
            if (computed.isNotEmpty()) {
                sha = computed
                isReal = true
                println("[FlexBoard obf] Bound constants to keystore SHA-256: $sha")
            } else {
                sha = "0".repeat(64)
                isReal = false
                println("[FlexBoard obf] Could not read keystore — falling back to plaintext mode.")
            }
        } else {
            sha = "0".repeat(64)
            isReal = false
            println("[FlexBoard obf] No keystore configured — generating plaintext (dev) constants.")
        }

        val seed = "flexboard_obf_v1::$pkgName::$sha".toByteArray(Charsets.UTF_8)
    ...
    buildTypes {
        release {
            ...
            signingConfig = if (canSignRelease) {
                ...
                println("[FlexBoard] No release keystore configured — release APK will be signed with the DEBUG key so it's still installable. Configure KEYSTORE_BASE64 secret in GitHub Actions for a production build.")
